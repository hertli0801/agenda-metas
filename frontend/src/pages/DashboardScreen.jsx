...
export default function DashboardScreen({ goals, stats, user, onSelectGoal, onNavigate }) {
  return (
    <div className="flex flex-col h-full justify-between bg-slate-50">
      <div className="p-6 space-y-6 overflow-y-auto max-h-[764px] flex-grow">
        <div>
          <h2 className="text-3xl font-extrabold text-slate-900">Hola, {user || "Fernanda"}</h2>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between gap-4">
          <div className="relative w-24 h-24 flex items-center justify-center shrink-0">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
              ...
              <circle cx="18" cy="18" r="15.915" stroke="#5f56d6" strokeWidth="3.2" strokeDasharray={`${stats.avgProgress}, 100`} strokeLinecap="round" fill="none" />
            </svg>
            <div className="absolute text-xl font-black text-slate-900">{stats.avgProgress}%</div>
          </div>
          ...
          <div className="flex-grow space-y-1.5">
            ...
            <div className="flex justify-between items-center text-xs text-slate-600 font-bold">
              <span>Metas Activas</span> <span className="bg-indigo-50 text-[#5f56d6] px-2 py-0.5 rounded-full text-[10px]">{stats.active}</span>
            </div>
            <div className="flex justify-between items-center text-xs text-slate-600 font-bold">
              <span>Completadas</span> <span className="bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full text-[10px]">{stats.completed}</span>
            </div>
          </div>
        </div>
              ...
              <div key={goal.id} onClick={() => onSelectGoal(goal)} className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-xs hover:shadow-md transition-all cursor-pointer space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-medium">Fecha límite: {goal.dueDate.split("-").reverse().join("/")}</p>
                  </div>
